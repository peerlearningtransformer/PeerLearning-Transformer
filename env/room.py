...
class RoomEnv(Hallway):
    ...
    def __init__(self, length=9, dims=1, **kwargs):
        ...
        self.observation_space = gym.spaces.Box(0, 1, shape=(2 * dims,),
                                                dtype=np.float64)
        # Note: self.np_random is initialized by gym.Env, don't override here

    def reset(self, seed=None, options=None):
        pos, _ = super().reset(seed=seed, options=options)
    ...
    def step(self, action: int):
        pos, reward, terminated, truncated, info = super().step(action)
