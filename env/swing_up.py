...
class CartPoleSwingUpEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.action_space = spaces.Box(-self.force_mag, self.force_mag,
                                       shape=(1,))
    ...
    def step(self, action: int):
        ...
        thetadot_update = (-3 * self.m_p_l * (
                    theta_dot ** 2) * s * c + 6 * self.total_m * self.g * s + 6 * (
                                       action - self.b * x_dot) * c) / (
...
gym.envs.registration.register(
    ...
    entry_point="env:CartPoleSwingUpEnv",
